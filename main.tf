...
resource "hcloud_server" "server_with_lifecycle_rules" {
  ...
  location           = (var.server_enable_public_ipv4 && var.server_public_ipv4_id) || (var.server_enable_public_ipv6 && var.server_public_ipv4_id) ? split("-", var.server_datacenter)[0] : var.server_location
  datacenter         = var.server_datacenter
